...
def plot_cls_loss_curve(run_dir):
    """
    Plot the DFL loss curves for train and validation, marking the best model
    ...
    # Check if DFL loss columns exist
    train_cls_col = [col for col in results_df.columns if 'train/cls_loss' in col]
    val_cls_col = [col for col in results_df.columns if 'val/cls_loss' in col]
    
    if not train_cls_col or not val_cls_col:
        print("DFL loss columns not found in results CSV")
        print(f"Available columns: {results_df.columns.tolist()}")
        return
    
    train_cls_col = train_cls_col[0]
    val_cls_col = val_cls_col[0]
    
    # Find the epoch with the best validation loss
    best_epoch = results_df[val_cls_col].idxmin()
    best_val_loss = results_df.loc[best_epoch, val_cls_col]
    
    # Create the plot
    plt.figure(figsize=(10, 6))
    
    # Plot training and validation losses
    plt.plot(results_df['epoch'], results_df[train_cls_col], label='Train CLS Loss')
    plt.plot(results_df['epoch'], results_df[val_cls_col], label='Validation CLS Loss')
    ...
    # Save the plot in the same directory as weights
    plot_path = os.path.join(run_dir, 'cls_loss_curve.png')
    plt.savefig(plot_path)
    
    # Also save it to the working directory for easier access
    plt.savefig(os.path.join('output','cls_loss_curve.png'))
