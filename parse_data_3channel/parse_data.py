    ...
    def process_tomogram_set(tomogram_ids, images_dir, labels_dir, set_name):
        ...
        for tomo_id, z_center, y_center, x_center, z_max in tqdm(motor_counts, desc=f"Processing {set_name} motors"):
            ...
            for z in range(z_min, z_max + 1):
                ...
                slice_filename_m1 = f"slice_{(z-1):04d}.jpg"
                slice_filename_p1 = f"slice_{(z+1):04d}.jpg"
                
                # Source path for the slice
                src_path = os.path.join(train_dir, tomo_id, slice_filename)
                src_path_m1 = os.path.join(train_dir, tomo_id, slice_filename_m1)
                src_path_p1 = os.path.join(train_dir, tomo_id, slice_filename_p1)
                
                if not os.path.exists(src_path):
                    print(f"Warning: {src_path} does not exist, skipping.")
                    continue
                if not os.path.exists(src_path_m1):
                    print(f"Warning: {src_path_m1} does not exist, skipping.")
                    continue
                if not os.path.exists(src_path_p1):
                    print(f"Warning: {src_path_p1} does not exist, skipping.")
                    continue
                
                # Load and normalize the slice
                img = Image.open(src_path)
                img_m1 = Image.open(src_path_m1)
                img_p1 = Image.open(src_path_p1)
                
                # Normalize the image
                normalized_img = np.stack([normalize_slice(np.array(img_m1)), 
                                           normalize_slice(np.array(img)), 
                                           normalize_slice(np.array(img_p1))], axis=-1)
